...
describe("GET /api/audit/logs - Filter by User", () => {
	...
	beforeAll(async () => {
		...
		// Create audit logs for target user
		for (let i = 0; i < 3; i++) {
			const logId = uuidv4();
		...
		// Create audit logs for another user
		for (let i = 0; i < 2; i++) {
			const logId = uuidv4();
			await AuditLog.create({
				_id: logId,
	...
	it("filters logs by userId parameter", async () => {
		const response = await request(app)
			.get(`/api/audit/logs?userId=${targetUserId}`)
