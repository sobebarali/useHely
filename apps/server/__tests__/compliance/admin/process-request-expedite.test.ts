...
describe("PUT /api/compliance/requests/:requestId/process - Expedite request", () => {
	...
	beforeAll(async () => {
		...
		// Create a pending request to expedite
		requestId = uuidv4();
	...
	it("expedites a pending request with COMPLIANCE:MANAGE permission", async () => {
		const response = await request(app)
			.put(`/api/compliance/requests/${requestId}/process`)
			.set("Authorization", `Bearer ${accessToken}`)
			.send({
				action: "expedite",
				notes: "Expedited for urgent request",
			});
