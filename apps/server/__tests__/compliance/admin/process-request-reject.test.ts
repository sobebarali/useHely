...
describe("PUT /api/compliance/requests/:requestId/process - Reject request", () => {
	...
	it("rejects a pending request with COMPLIANCE:MANAGE permission", async () => {
		...
		expect(response.body.data.status).toBe(DataSubjectRequestStatus.CANCELLED);
		...
		expect(updatedRequest?.status).toBe(DataSubjectRequestStatus.CANCELLED);
	});
});
