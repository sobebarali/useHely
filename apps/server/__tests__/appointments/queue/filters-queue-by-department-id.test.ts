...
describe("GET /api/appointments/queue - Filters queue by department ID", () => {
	...
	beforeAll(async () => {
		...
		for (let i = 0; i < 3; i++) {
			const appointment = await Appointment.create({
				...
				status: "CHECKED_IN",
				queueNumber: i + 1,
				checkedInAt: new Date(Date.now() - (3 - i) * 60000),
				createdAt: new Date(),
				updatedAt: new Date(),
			});
	...
	it("filters queue by departmentId and returns matching appointments", async () => {
