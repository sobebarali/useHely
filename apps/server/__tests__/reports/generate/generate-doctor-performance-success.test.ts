...
describe("POST /api/reports/generate - Doctor performance success", () => {
	...
	it("generates doctor performance report with date range", async () => {
		...
		expect(response.body.reportType).toBe("doctor-performance");
	...
	it("returns correct data structure for doctor performance", async () => {
		const response = await request(app)
			.post("/api/reports/generate")
			.set("Authorization", `Bearer ${accessToken}`)
			.send({
	...
	it("generates with specific doctor filter", async () => {
		const response = await request(app)
			.post("/api/reports/generate")
			.set("Authorization", `Bearer ${accessToken}`)
			.send({
