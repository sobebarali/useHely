...
describe("PATCH /api/patients/:id - Unauthorized access", () => {
	...
	beforeAll(async () => {
		// Create context WITH full permissions to create test data
		...
		const payload = {
			...
			gender: "MALE",
			phone: `+1-unauth-upd-${contextWithPermission.uniqueId}`,
			address: {
				street: "123 Main St",
				city: "Test City",
				state: "TS",
		...
		// Create context WITHOUT PATIENT:UPDATE permission
		contextWithoutPermission = await createAuthTestContext({
			rolePermissions: ["PATIENT:READ"], // Only read, no update
			includeDepartment: true,
		});
