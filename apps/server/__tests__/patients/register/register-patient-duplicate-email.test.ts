...
describe("POST /api/patients - Duplicate email error", () => {
	let context: AuthTestContext;
	let accessToken: string;
	let createdPatientId: string;
	const testEmail = `dup-email-${Date.now()}@test.com`;
	...
	beforeAll(async () => {
		...
		// Create a patient with an email first
		const payload = {
			firstName: "Existing",
			lastName: "Patient",
			dateOfBirth: "1990-05-15T00:00:00.000Z",
			gender: "MALE",
			phone: `+1-dup-email-${context.uniqueId}`,
			email: testEmail,
			address: {
				street: "123 Main St",
				city: "Test City",
	...
	it("returns 409 when email already exists in tenant", async () => {
		const payload = {
			...
			gender: "FEMALE",
			phone: `+1-new-phone-${context.uniqueId}`,
			email: testEmail, // Same email as existing patient
			address: {
				street: "456 Other St",
				city: "Other City",
				state: "OS",
